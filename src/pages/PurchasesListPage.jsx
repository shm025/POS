  ...
  return (
    <div className="page-view">
      <div className="flex-between mb-4 no-print">
        <h1 style={{ fontSize:'20px', fontWeight:900, color:'var(--primary)' }}>🛒 سجل المشتريات</h1>
        <button className="btn btn-primary" onClick={() => navigate('/purchases')}>➕ فاتورة شراء جديدة</button>
      </div>
      <div className="card">
        <div className="search-bar no-print">
          <input className="form-control" placeholder="🔍 بحث بالرقم أو المورد..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <div className="table-wrapper">
          <table>
            <thead>
              <tr><th>الرقم</th><th>المورد</th><th>التاريخ</th><th>الاستحقاق</th><th>الإجمالي</th><th>الحالة</th><th className="no-print">إجراء</th></tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan="7"><div className="empty-state"><div className="icon">🛒</div><p>لا توجد فواتير مشتريات</p></div></td></tr>
              ) : filtered.map(p => (
                <tr key={p.id}>
                  ...
                  <td className="no-print">
                    <button className="btn btn-sm btn-outline" onClick={() => navigate('/purchases/edit/' + p.id)}>✏️ تعديل</button>
