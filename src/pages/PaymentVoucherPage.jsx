...
export default function PaymentVoucherPage() {
  ...
  useEffect(() => {
    loadVouchers('payment').then(v => {
      setForm(p => ({ ...p, number: `PAY-2026-${String(v.length + 1).padStart(3, '0')}` }))
    })
  }, [loadVouchers])
  ...
  async function handleSave() {
    ...
    loadVouchers('payment').then(v => {
      setForm(p => ({ ...p, party:'', amount:0, desc:'', number: `PAY-2026-${String(v.length + 1).padStart(3, '0')}` }))
    })
  }
  ...
  return (
    <div className="page-view">
      <div className="flex-between mb-4 no-print">
        <h1 style={{ fontSize:'20px', fontWeight:900, color:'var(--primary)' }}>💸 سند صرف</h1>
      </div>
      <div className="grid-2">
        <div className="card">
          <div className="card-header"><div className="card-title">💸 سند صرف جديد</div></div>
          ...
          <div className="form-group">
            <label className="form-label">المصروف إليه</label>
        ...
        <div className="card">
          ...
          <div className="table-wrapper">
            <table>
              <thead><tr><th>رقم</th><th>إلى</th><th>المبلغ</th><th>التاريخ</th></tr></thead>
              <tbody>
